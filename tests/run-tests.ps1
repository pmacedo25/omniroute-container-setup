...
Import-Module (Join-Path $projectDirectory "scripts\OpenRouterAI.Setup.psm1") -Force -DisableNameChecking
...
try {
    ...
    $openRouterConfig = Write-OpenRouterAIConfiguration -HomeDirectory $testRoot `
        -ProjectsDirectory "C:\Users\test\workspace" -OmniRoutePort 20128
    $openRouterSettings = Get-Content -LiteralPath $openRouterConfig -Raw | ConvertFrom-Json
    Assert-Equal "http://127.0.0.1:20128/v1" $openRouterSettings.omnirouteBaseUrl "IDE deve usar loopback do host"
    Assert-Equal "OMNIROUTE_API_KEY" $openRouterSettings.apiKeyEnvironmentVariable "Config não deve duplicar segredo"
    Assert-Equal 3 @($openRouterSettings.allowedCombos).Count "IDE deve permitir somente três combos"
    Assert-True (-not (Get-Content -LiteralPath $openRouterConfig -Raw).Contains("sk-")) "Config da IDE não deve conter chave"
    Assert-Equal "0.1.0" (Get-OpenRouterAIVersion -ArtifactPath "OpenRouterAI-win-x64-0.1.0.zip" -ManifestPath "") "Versão deve vir do artefato"
    $artifactPath = Join-Path $testRoot "OpenRouterAI-win-x64-0.1.0.zip"
    ...
    Set-Content -LiteralPath $sumsPath -Value "$artifactHash  OpenRouterAI-win-x64-0.1.0.zip" -Encoding ascii
    Assert-Equal $artifactHash (Test-OpenRouterAIArtifact -ArtifactPath $artifactPath -Sha256SumsPath $sumsPath) "Checksum válido deve ser aceito"
    Set-Content -LiteralPath $sumsPath -Value "$("0" * 64)  OpenRouterAI-win-x64-0.1.0.zip" -Encoding ascii
    $checksumRejected = $false
    try {
        Test-OpenRouterAIArtifact -ArtifactPath $artifactPath -Sha256SumsPath $sumsPath | Out-Null
    } catch {
